...
public class SSHClient {
     ...
     */
    public String sendCmdAnd(String command, String... cmdExt) throws Exception {
    ...
    /**
     *  如果每个命令被双竖线 || 所分隔，那么一遇到可以执行成功的命令就会停止执行后面的命令，而不管后面的命令是否正确与否。
     *  如果执行到错误的命令就是继续执行后一个命令，一直执行到遇到正确的命令为止。
     */
    public String sendCmdOr(String command, String... cmdExt) throws Exception {
        StringBuilder cmd = new StringBuilder(command);
        for (String ext : cmdExt) {
            cmd.append(" ").append("||").append(" ").append(ext);
        }
     ...
     */
    public String sendCmdStep(String command, String... cmdExt) throws Exception {
        StringBuilder cmd = new StringBuilder(command);
        for (String ext : cmdExt) {
            cmd.append(" ").append(";").append(" ").append(ext);
        }
